	...
	Context("when there is only one pot", func() {
		...
		Context("and a player has a winning hand", func() {
			...
			It("gives the whole pot to the winning player", func() {
				results, oddChip := Showdown([]*Player{charlie, dennis, dee, mac}, []*Pot{onlyPot})
				Expect(*results[charlie][0]).To(Equal(*onlyPot))
				Expect(results[dennis]).To(BeNil())
				Expect(oddChip).To(Equal(0))
			})
		})
	})
})
